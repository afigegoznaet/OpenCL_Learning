...
int main() {
	...
	err =
		clGetKernelWorkGroupInfo(kernel_init, device, CL_KERNEL_WORK_GROUP_SIZE,
	...
	err |=
		clSetKernelArg(kernel_stage_0, 1, 8 * local_size * sizeof(float), NULL);
	err |=
		clSetKernelArg(kernel_stage_n, 1, 8 * local_size * sizeof(float), NULL);
	err |=
		clSetKernelArg(kernel_merge, 1, 8 * local_size * sizeof(float), NULL);
	err |= clSetKernelArg(kernel_merge_last, 1, 8 * local_size * sizeof(float),
						  NULL);
	if (err < 0) {
		printf("Couldn't set a kernel argument");
		exit(1);
	...
	for (stage = num_stages; stage > 1; stage >>= 1) {
		...
		err = clEnqueueNDRangeKernel(queue, kernel_merge, 1, NULL, &global_size,
									 &local_size, 0, NULL, NULL);
		if (err < 0) {
			perror("Couldn't enqueue the kernel");
			exit(1);
	...
	/* Read the result */
	err = clEnqueueReadBuffer(queue, data_buffer, CL_TRUE, 0, sizeof(data),
							  &data, 0, NULL, NULL);
	if (err < 0) {
