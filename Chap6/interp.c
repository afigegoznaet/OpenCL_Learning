...
cl_device_id create_device() {
	...
	err = clGetDeviceInfo(dev, CL_DEVICE_NAME, 48 * sizeof(char), name_data,
						  NULL);
	if (err < 0) {
...
void read_image_data(const char *filename, png_bytep *input, png_bytep *output,
					 size_t *w, size_t *h) {
	...
	*output = malloc(2 * (*h) * png_get_rowbytes(png_ptr, info_ptr)
					 * SCALE_FACTOR * SCALE_FACTOR);
	for (i = 0; i < *h; i++) {
		png_read_row(png_ptr, *input + i * png_get_rowbytes(png_ptr, info_ptr),
					 NULL);
	}
	...
	printf("Print!\n");
	// printf("Bit depth: %d\nColor type: %d\nPixel depth:
	// %d\n",info_ptr->bit_depth, info_ptr->color_type, info_ptr->pixel_depth);
}
...
int main(int argc, char **argv) {
	...
	/*
		...
		image_desc.buffer = (void *)input_pixels;
		/*
		input_image =
			clCreateImage(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
						  &png_format, &image_desc, NULL, &err);
		/**/
	/**/
	input_image = clCreateImage2D(
		context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, &png_format, width,
		height, 0, (void *)input_pixels, &err);
	/**/
	if (err < 0) {
	...
	output_image = clCreateImage2D(context, CL_MEM_WRITE_ONLY, &png_format,
								   SCALE_FACTOR * width, SCALE_FACTOR * height,
								   0, NULL, &err);

	printf("Putput width: %zd, height: %zd\n", SCALE_FACTOR * width,
		   SCALE_FACTOR * height);
	...
	write_image_data(OUTPUT_FILE, output_pixels, SCALE_FACTOR * width,
					 SCALE_FACTOR * height);
