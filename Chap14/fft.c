...
int main() {
	...
	data_buffer = clCreateBuffer(context, CL_MEM_READ_WRITE,
								 2 * NUM_POINTS * sizeof(float), NULL, &err);
	if (err < 0) {
		printf("Couldn't create a buffer: %d\n", err);
		exit(1);
	};
	...
	err =
		clGetKernelWorkGroupInfo(init_kernel, device, CL_KERNEL_WORK_GROUP_SIZE,
	...
	err |= clSetKernelArg(init_kernel, 3, sizeof(points_per_group),
						  &points_per_group);
	...
	if (num_points > points_per_group) {
		...
		err |= clSetKernelArg(stage_kernel, 2, sizeof(points_per_group),
							  &points_per_group);
		...
		for (stage = 2; stage <= num_points / points_per_group; stage <<= 1) {
			...
			err = clEnqueueNDRangeKernel(queue, stage_kernel, 1, NULL,
	...
	if (direction < 0) {
		...
		err |= clSetKernelArg(scale_kernel, 1, sizeof(points_per_group),
							  &points_per_group);
	...
	err = clEnqueueReadBuffer(queue, data_buffer, CL_TRUE, 0,
							  2 * NUM_POINTS * sizeof(float), data, 0, NULL,
							  NULL);
	if (err < 0) {
	...
	error = 0.0;
	for (i = 0; i < NUM_POINTS; i++) {
		error += fabs(check_output[i][0] - data[2 * i])
				 / fmax(fabs(check_output[i][0]), 0.0001);
		error += fabs(check_output[i][1] - data[2 * i + 1])
				 / fmax(fabs(check_output[i][1]), 0.0001);
	}
