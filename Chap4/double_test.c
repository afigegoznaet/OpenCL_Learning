...
int main() {
	...
	if (clGetDeviceInfo(device, CL_DEVICE_ADDRESS_BITS, sizeof(addr_data),
						&addr_data, NULL)
		< 0) {
	...
	clGetDeviceInfo(device, CL_DEVICE_EXTENSIONS, sizeof(ext_data), NULL,
					&ext_size);
	ext_data = (char *)malloc(4096);
	ext_data[ext_size] = '\0';
	clGetDeviceInfo(device, CL_DEVICE_EXTENSIONS, 4096, ext_data, NULL);
	...
	b_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
							  sizeof(float), &b, NULL);
	output_buffer =
		clCreateBuffer(context, CL_MEM_WRITE_ONLY, sizeof(float), NULL, NULL);
	...
	err = clEnqueueReadBuffer(queue, output_buffer, CL_TRUE, 0, sizeof(float),
							  &result, 0, NULL, NULL);
	if (err < 0) {
