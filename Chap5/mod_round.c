...
cl_device_id create_device() {
	...
	clGetDeviceInfo(dev, CL_DEVICE_SINGLE_FP_CONFIG, sizeof(int), &flags, NULL);
	printf("The flag is set: %d\n", flags & CL_FP_FMA);
	return dev;
}
...
int main() {
	...
	cl_mem mod_input_buffer, mod_output_buffer, round_input_buffer,
		round_output_buffer;
	...
	/* Display data */
	printf("fmod(%.1f, %.1f)      = %.1f\n", mod_input[0], mod_input[1],
		   mod_output[0]);
	printf("remainder(%.1f, %.1f) = %.1f\n\n", mod_input[0], mod_input[1],
		   mod_output[1]);

	printf("Rounding input: %.1f %.1f %.1f %.1f\n", round_input[0],
		   round_input[1], round_input[2], round_input[3]);
	printf("rint:  %.1f, %.1f, %.1f, %.1f\n", round_output[0], round_output[1],
		   round_output[2], round_output[3]);
	printf("round: %.1f, %.1f, %.1f, %.1f\n", round_output[4], round_output[5],
		   round_output[6], round_output[7]);
	printf("ceil:  %.1f, %.1f, %.1f, %.1f\n", round_output[8], round_output[9],
		   round_output[10], round_output[11]);
	printf("floor: %.1f, %.1f, %.1f, %.1f\n", round_output[12],
		   round_output[13], round_output[14], round_output[15]);
	printf("trunc: %.1f, %.1f, %.1f, %.1f\n", round_output[16],
		   round_output[17], round_output[18], round_output[19]);
