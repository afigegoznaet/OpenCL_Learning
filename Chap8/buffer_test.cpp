...
int main(void) {
	...
	try {
		...
		queue.enqueueWriteBuffer(matrixBuffer, CL_TRUE, 0, sizeof(fullMatrix),
								 fullMatrix);
		...
		region[1] = (4);
		region[2] = (1);
		queue.enqueueReadBufferRect(matrixBuffer, CL_TRUE, bufferOrigin,
									hostOrigin, region, 10 * sizeof(float), 0,
